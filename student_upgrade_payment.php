    ...
    if ($gateway === 'none' || !is_gateway_ready()) {
        ...
        if ($chargeResult && $chargeResult['success']) {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, 'active', 'paid', ?, ?, ?, ?)
            ");
            $stmt->execute([current_school_id(), $student_id, $new_plan_id, $start_date, $end_date, $totalWithFees, $auto_renew, $billing_day, $monthly_charges]);
            ...
            if ($amountCharged > 0) {
                ...
                ");
                $stmt->execute([current_school_id(), $student_id, $membership_id, $amountCharged, generateReceiptNumber(), $notes]);
            }
            ...
            recordAllDiscountCodeUses($feeBreakdown, $student_id, 'upgrade', $membership_id);
...
(function() {
    ...
    var appliedCodes = [];
    var originalBreakdownHtml = '<?php echo addslashes(renderFeeBreakdownHtml($feeBreakdown, true, $isFirstEnrollment ? ($isMonthlyPlan ? "First Monthly Payment" : "Amount Due") : "Subtotal")); ?>';
    ...
    window.removeCode = function(code) {
        ...
        if (appliedCodes.length === 0) {
            ...
            if (btnText) btnText.textContent = 'Pay Now - $' + parseFloat(<?php echo json_encode($feeBreakdown['total']); ?>).toFixed(2);
            renderCodeBadges([]);
        } else {
            revalidateAll();
        }
